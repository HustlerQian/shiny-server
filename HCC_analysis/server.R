...
shinyServer(function(input, output) {
  ...
  exprs.m<-reactive({
    if(input$data_type=='methy'){
      ...
      data=read.csv('./HCC_1018_methy.csv',header = T)
      METHY_MAT=data[,-1]
    ...
    if(input$data_type=='exp'){
      ...
      data=read.csv('./HCC_1018_exp.csv',header = T)
  ...
  anotation.m<-reactive({
    ...
    ano_mat=read.table('./clinical_patient_HCC.txt',header = T,sep = '\t',quote='')
    ANO_COL=ano_mat[,c(-1)]
  ...
  output$downloadData<- downloadHandler(
    filename = function() { 
      paste(paste0('HCC_',input$phenotype_test), '.csv', sep='') 
    },
