...
shinyServer(function(input, output) {
  ...
  exprs.m<-reactive({
      if(input$data_type=='methy'){
        ...
        data=read.csv('./BTC_346_methy.csv',header = T)
        METHY_MAT=data[,-1]
      ...
      if(input$data_type=='exp'){
        ...
        data=read.csv('./BTC_346_exp.csv',header = T)
  ...
  anotation.m<-reactive({
    ...
    ano_mat=read.table('./clinical_patient_BTC&CCA.txt',header = T,sep = '\t',quote='')
    ANO_COL=ano_mat[,c(-1)]
  ...
  output$downloadData<- downloadHandler(
    filename = function() { 
      paste(paste0('BTC_',input$phenotype_test), '.csv', sep='') 
    },
