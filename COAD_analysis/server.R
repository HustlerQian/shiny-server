...
shinyServer(function(input, output) {
  ...
  exprs.m<-reactive({
      if(input$data_type=='methy'){
        ...
        data=read.csv('./COAD_1918_methy.csv',header = T)
        METHY_MAT=data[,-1]
        rownames(METHY_MAT)=data[,1]
        colnames(METHY_MAT)=substr(colnames(METHY_MAT),1,12)
        #Excluding the Patientid equals to 
        #AAT5,AATA
        #6317 02A /A8F7 02A
        METHY_MAT <- METHY_MAT[, !colnames(METHY_MAT) %in% c("TCGA.5M.AAT5","TCGA.5M.AATA","TCGA.G4.6317.1","TCGA.NH.A8F7.1")]
        return(METHY_MAT)
      }
      if(input$data_type=='exp'){
        #Load expression matrix
        data=read.csv('./COAD_1918_exp.csv',header = T)
        ...
        #AAT5,AATA
        #6317 02A /A8F7 02A
        EXP_MAT <- EXP_MAT[, !colnames(EXP_MAT) %in% c("TCGA.5M.AAT5","TCGA.5M.AATA","TCGA.G4.6317.1","TCGA.NH.A8F7.1")]
        return(EXP_MAT)
      }
  })
  anotation.m<-reactive({
    #Load anotation matrix
    ano_mat=read.table('./clinical_patient_COAD.txt',header = T,sep = '\t',quote='')
    ANO_COL=ano_mat[,c(-1)]
  ...
  output$downloadData<- downloadHandler(
    filename = function() { 
      paste(paste0('COAD_',input$phenotype_test), '.csv', sep='') 
    },
